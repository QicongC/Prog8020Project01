...
class rowling_recent_comments extends WP_Widget {
	...
	function __construct() {
        $widget_ops = array( 'classname' => 'widget_rowling_recent_comments', 'description' => __('Displays recent comments with user avatars.', 'rowling') );
        parent::__construct( 'widget_rowling_recent_comments', __('Recent Comments','rowling'), $widget_ops );
    }
					...
					if ( $comments ) {
						foreach ( $comments as $comment ) { ?>
							...
							<li>
								...
								<a href="<?php echo get_permalink($comment->comment_post_ID); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php _e('Comment to','rowling'); echo ' ' . get_the_title($comment->comment_post_ID) . ', '; _e('posted','rowling'); echo ' ' . get_the_time(get_option('date_format')); ?>">
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('number_of_comments'); ?>"><?php _e('Number of comments to display', 'rowling'); ?>:
